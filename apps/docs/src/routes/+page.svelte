<script lang="ts">
	...
	import { ChatBubble } from '@/components/index.js';

	let selectedPortNum = $state<number | null>(null);
</script>

<Section name="0. Ensure prerequisites">
...
<Section name="1. (Optional) start supabase local instance">
	<ChatBubble>
	...
	></Code>
	<ChatBubble>
		Please pay attention to the 5th & 6th lines, those are critical information for the next step.
	</ChatBubble>
</Section>

<Section name="2. (Optional) set environment variables">
	<article>
		If you're developing one of these apps:
		{@html APPS.filter(({ dbRelated }) => dbRelated)
			.map(({ name }) => `<a href="/apps/${name}" class="link link-hover link-primary">${name}</a>`)
...
<Section name="3. Start the dev server">
	<Code
		lines={[
			'# at project root',
...
<Section name="4. Accessing svelte apps through browser">
