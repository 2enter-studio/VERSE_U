...
<div class="flex flex-row full-screen">
		<div class="w-96 gap-1 backdrop-blur-sm bg-white/5 shadow-inner shadow-black/40 rounded-lg m-4 overflow-scroll p-2 h-[calc(100%-2rem)]">
				...
				<div class="collapse collapse-plus">
					...
					<div class="collapse-content">
						<div class="flex flex-rol mb-4 items-center">
								<span class="rounded bg-white text-center p-2 flex items-center mr-4 text-info-content text-xs">search (id)</span>
							<input type="text" class="input input-bordered input-sm max-w-xs" bind:value={searchIds[tableName]} />
						</div>
						<div class="flex flex-col gap-1">
							{#each tablesData[tableName].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()) as row}
								...
								<div
									class="flex flex-row mb-1 {id.includes(searchIds[tableName] ?? '') || !searchIds[tableName]
										? ''
										: 'hidden'}"
								>
									<button
						...
						{#if !TABLES_INFO[tableName].readonly}
							<button class="btn btn-sm btn-accent w-full text-white" onclick={() => onAdd(tableName)}>Add +</button>
						{/if}
					</div>
				</div>
			{/each}
		...
		<div class="w-[calc(100%-424px)] h-full">
			<div class="backdrop-blur-sm bg-white/5 shadow-inner shadow-black/40 rounded-lg m-4 h-108 overflow-auto h-4/5 p-2">
				<h1 class="flex text-xl justify-center"> Editor - {$editing && TABLES_INFO[$editing.tableName].description } </h1>
				{#key $editing}
					{#if $editing?.id}
			...
			<div class="flex flex-col items-center m-4 p-2 backdrop-blur-sm bg-white/5 shadow-inner shadow-black/40 rounded-lg h-[calc(20%-3rem)]">
