		...
		<div class="flex flex-col gap-3 items-start text-center w-full h-full p-4 overflow-y-auto">
			{#each Object.entries(metadata) as [name, content]}
				...
				<div class="flex items-start gap-0.5 {isMLTexts ? 'flex-col' : 'flex-row flex-wrap'}">
					{#if !isMLTexts}
						<h2 class="rounded text-center p-2 flex items-center mr-4 text-white text-bold bg-orange-600">{name}</h2>
						{#if content.readonly}
							<Form
								data={data[name]}
								{name}
						...
						{/if}
					{:else}
						<h2 class="rounded text-center p-2 flex items-center mr-4 text-white text-bold  bg-cyan-600">{name}</h2>
				...
				{#each Object.entries(tableInfo.reference) as [name, content]}
					<div class="flex items-start gap-0.5 flex-col">
						...
						{:else if content.type === 'multi_ref'}
							<h2 class="rounded text-center p-2 flex items-center mr-4 text-white text-bold  bg-cyan-700">{name}</h2>
							<Forms.multi_ref
								base={tableName}
								target={content.target}
								{name}
