...
{#if $chat}
	<div in:fly={{ x: isMine ? 100 : -100, duration: 70 }} class="my-[3px] flex w-full flex-row items-end {isMine ? 'justify-end' : 'justify-start'}">
		{#if !isMine}
			{@const profile = $chat.chat_members.find((m) => m.profiles.user === message.sender)?.profiles}
