<script lang="ts">
	import { chatId, chats, getMemberFromChat } from '../../lib/states';
...
<div class="flex h-full w-full flex-col gap-2 rounded-t-2xl border-x-[5px] border-orange-500 bg-yellow-100 px-2 py-2">
	{#each chatrooms as chatroom}
		{@const person = getMemberFromChat(chatroom)?.profiles}
		<button
			class="flex flex-row items-center gap-1 rounded-xl bg-amber-400 text-black shadow-inner shadow-amber-700"
			onclick={() => ($chatId = chatroom.id)}
		>
