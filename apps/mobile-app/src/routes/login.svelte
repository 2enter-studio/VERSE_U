<script lang="ts">
	...
	const formChoices = $derived($loggedIn ? (['change_pwd', 'forgot_pwd'] as const) : (['login'] as const));
	...
	const submittable = $derived(
		(pwd === pwdConfirm && validate.password(pwd) && formMode === 'signup') ||
			...
			(pwdNew === pwdConfirm && validate.password(pwdNew) && pwdNew !== pwd && formMode === 'change_pwd') ||
	...
	const formFields = $derived.by<InputType[]>(() => {
		...
		else if (formMode === 'change_pwd') return ['password', 'new_password', 'confirm_password'] as const;
		return [];
	});
	...
	const inputClasses = 'rounded-md text-sm py-0.5 w-full bg-black text-white border-white border-[1px] pl-2';
</script>
...
<div class="center-content w-[60vw] flex-col gap-1">
	...
	<div class="flex w-full flex-row justify-between gap-2 rounded-md bg-black/90 p-1 shadow-inner shadow-white/50">
	...
	<form onsubmit={handleSubmit} class="flex w-full flex-col items-end rounded-lg bg-white px-3 py-3 text-black shadow-inner shadow-black/40">
		{#each formFields as field}
			<div transition:slide={{ duration: 500 }} class="flex w-full flex-col">
				...
				{#if field === 'email'}
					<input id="email" type="email" class={inputClasses} bind:value={email} readonly={$loggedIn} required />
				{/if}
				{#if field === 'password'}
					<div class="flex w-full flex-row items-center gap-1">
						<input type={pwdVisible ? 'text' : 'password'} class={inputClasses} bind:value={pwd} required />
				...
				{#if field === 'new_password'}
					<input type={pwdVisible ? 'text' : 'password'} class={inputClasses} bind:value={pwdNew} required />
				{/if}
				{#if field === 'confirm_password'}
					<input type={pwdVisible ? 'text' : 'password'} class={inputClasses} bind:value={pwdConfirm} required />
				{/if}
			</div>
		{/each}
		<!--{#if submittable}-->
		<button type="submit" class="center-content mt-2" disabled={!submittable}>
			<Icon
				...
				class="size-8 rounded-full p-0 {submittable ? 'text-red-500' : 'text-red-500/50'} shadow-inner shadow-amber-800"
			/>
		</button>
		<!--{/if}-->
		<div class="center-content mt-3 w-full">
			<div class="flex w-fit flex-col justify-start rounded-lg bg-green-800 px-3 py-1 text-xs text-white">
				<h5>Demo Account</h5>
	...
	{#if !$loggedIn && OAUTH_PROVIDERS.length > 0}
		<div class="mt-2 flex flex-row justify-around gap-5 rounded-full bg-red-500 p-1 shadow-inner shadow-red-800/80">
			{#each OAUTH_PROVIDERS as provider}
				...
				>
					<Icon icon="mdi:{provider}" class="center-content size-9 rounded-full bg-white p-2 text-black shadow-inner shadow-black/20" />
				</button>
			{/each}
		</div>
	{/if}
