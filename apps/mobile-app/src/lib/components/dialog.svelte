<script lang="ts">
	...
	type Props = { children: Snippet; title: string; open: boolean; class?: string; closable?: boolean; onclose?: () => void };
	let { children, title, open = $bindable<boolean>(), class: className, closable = true, onclose }: Props = $props();
		...
		<div transition:flyAndScale class="center-content h-full w-full flex-col">
			{#if closable}
				<div class="flex w-9/12 justify-start">
					<button class="center-content size-6 rounded-full border-b-2 border-r-2 border-rose-800 bg-rose-600" onclick={close}>
						<Icon icon="iconamoon:close-bold" />
					</button>
				</div>
			{/if}
			<h1 class="rounded-t-xl border-r-4 border-cyan-800 bg-cyan-700 px-3 py-1 font-bold">{title}</h1>
			<div
